...
func calculate(wg *sync.WaitGroup, performance chan float64) {
	...
	f, err := os.OpenFile("/home/hstsai/onvm/result/total.csv", os.O_APPEND|os.O_WRONLY|os.O_CREATE, 0666)
	defer f.Close()
...
func calculateWrite(wg *sync.WaitGroup, performanceWrite chan float64) {
	...
	f, err := os.OpenFile("/home/hstsai/onvm/result/write.csv", os.O_APPEND|os.O_WRONLY|os.O_CREATE, 0666)
	defer f.Close()
...
func calculateRead(wg *sync.WaitGroup, performanceRead chan float64) {
	...
	f, err := os.OpenFile("/home/hstsai/onvm/result/read.csv", os.O_APPEND|os.O_WRONLY|os.O_CREATE, 0666)
	defer f.Close()
...
func client(num int, wg *sync.WaitGroup, performance chan float64, performanceRead chan float64, performanceWrite chan float64) {
	...
	t1 := time.Now()
	_, tW, tR, err := sendTCP("127.0.0.3:8591", msg)
	t2 := time.Now()
